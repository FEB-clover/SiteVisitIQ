...
export async function buildReport({ type, property, floors = [], items = [], item = null, mapBytes = null, floorBytes = [], photoBytes = [], photoSets = null, report = null, date }) {
  ...
  if (type === 'sitevisit') {
    ...
    const IC = (pg, img, x, yTop, w, h) => {
      const by = H - yTop - h;
      if (!img) { pg.drawRectangle({ x, y: by, width: w, height: h, color: T.soft }); return; }
      const s = Math.max(w / img.width, h / img.height);
      const dw = img.width * s, dh = img.height * s;
      pg.pushOperators(pushGraphicsState(), rectangle(x, by, w, h), clip(), endPath());
      pg.drawImage(img, { x: x - (dw - w) / 2, y: by - (dh - h) / 2, width: dw, height: dh });
    ...
    list.forEach((it, i) => {
      ...
      TX(cov, String(i + 1).padStart(2, '0'), MG, y, 8.5, bold, T.accent);
      tl.forEach((ln, k) => TX(cov, ln, TCOL, y + k * 10.5, 8.5, reg, T.ink));
      TX(cov, it.category || '—', CATX, y, 7.5, reg, T.warm);
      CAPS(cov, it.life_safety ? 'LIFE SAFETY' : rating(it), SEVX, y, 6, urgent(it) ? T.flag : T.warm, 1.2);
      y += Math.max(14, tl.length * 10.5 + 4);
      RULE(cov, MG, y - 3, W - MG); y += 7;
    });
    foot(cov);
    ...
    const measureBlock = (wid, it, photoH) => {
      let h = 13;
      h += Math.min(2, wrapLines(it.title || '(untitled)', bold, 10.5, wid - 36).length) * 12.5 + 1;
      if (it.category || it.walker_name) h += 11;
      h += 8;
      if (it.notes) h += 9 + Math.min(3, wrapLines(it.notes, reg, 8, wid).length) * 9.8 + 4;
      if (it.detail) h += 9 + Math.min(3, wrapLines(it.detail, reg, 8, wid - 10).length) * 9.8 + 4;
      return h + 2 + photoH;
    };
    ...
    const block = (x, yTop, wid, it, idx, photoH, perRow) => {
      let ty = yTop;
      const IND = 36;
      ...
      ty += 13;
      wrapLines(it.title || '(untitled)', bold, 10.5, wid - IND).slice(0, 2)
      ...
      if (nt) {
        ...
        wrapLines(nt, reg, 8, wid).slice(0, 3).forEach((ln) => { TX(pg, ln, x, ty, 8, reg, T.ink); ty += 9.8; });
        ty += 4;
      }
      // MORE DETAIL — added at the office, marked with an accent rule
      const dt = it.detail || '';
      if (dt) {
        const dl = wrapLines(dt, reg, 8, wid - 10).slice(0, 3);
      ...
      // photos flush at the foot of the block
      const ims = imgs[it.id] || [];
      const py = yTop + (REC ? 0 : 0);
      const top = Math.max(ty + 2, yTop);
      const n = Math.max(1, Math.min(ims.length, perRow));
      const sw = (wid - 5 * (n - 1)) / n;
      if (!ims.length) IC(pg, null, x, top, wid, photoH);
      else ims.slice(0, n).forEach((im, k) => IC(pg, im, x + k * (sw + 5), top, sw, photoH));
      return top + photoH;
    };
    ...
    const PH = REC ? 124 : 70;
    const PERROW = REC ? 2 : 3;
    const GW = (CW - 20) / 2;
    const CARDH = Math.max(...list.map((it) => measureBlock(GW, it, PH))) + 10;
    let i = 0; y = open();
    while (i < list.length) {
      if (y + CARDH > LIMIT) y = open();
      for (let c = 0; c < 2 && i < list.length; c++, i++) {
        block(MG + c * (GW + 20), y, GW, list[i], i, PH, PERROW);
      }
      y += CARDH;
      if (y < LIMIT) RULE(pg, MG, y - 12, W - MG);
    }
    return doc.save();
  }
  // ---------- AGENDA / CRITICAL ----------
