...
function drawHeader(page, fonts, { title, property, date, extra = [] }) {
  ...
  page.drawText('OPERATIONS INTELLIGENCE', { x: M + 28, y: PAGE_H - 50, size: 7.5, font: bold, color: MUTED, characterSpacing: 1.4 });
...
export async function buildReport({ type, property, floors = [], items = [], item = null, mapBytes = null, floorBytes = [], photoBytes = [], photoSets = null, report = null, date }) {
  ...
  if (type === 'sitevisit') {
    ...
    // Photos are never cropped AND never shrink to squeeze onto one line.
    // Every photo in the report is drawn at the SAME height; width follows its
    // own proportions, and the row wraps when the next one will not fit. An
    // issue with five photos therefore gets two rows of full-size photos
    // rather than one row of unreadable ones.
    const GAP = 5;
    const photoLayout = (ims, wid, H) => {
      const rows = []; let row = [], used = 0;
      for (const im of ims) {
        let h = H, w = H * (im.width / im.height);
        if (w > wid) { w = wid; h = wid * (im.height / im.width); }   // panorama safety
        const need = (row.length ? GAP : 0) + w;
        if (row.length && used + need > wid) { rows.push(row); row = []; used = 0; }
        row.push({ im, w, h });
    ...
    const photoRowH = (ims, wid, H) => {
      if (!ims || !ims.length) return 0;
      const rows = photoLayout(ims, wid, H);
      return rows.reduce((t, r) => t + Math.max(...r.map((c) => c.h)), 0) + GAP * (rows.length - 1);
    };
    const photoRow = (pg, ims, x, yTop, wid, H) => {
      if (!ims || !ims.length) return 0;
      let y = yTop;
      for (const row of photoLayout(ims, wid, H)) {
        const rh = Math.max(...row.map((c) => c.h));
        let cx = x;
        for (const c of row) {
          pg.drawRectangle({ x: cx, y: PAGE_H - y - c.h, width: c.w, height: c.h, color: T.soft });
          pg.drawImage(c.im, { x: cx, y: PAGE_H - y - c.h, width: c.w, height: c.h });
          cx += c.w + GAP;
        }
        y += rh + GAP;
      }
    ...
    // the document's own Title is what most previewers show — iOS Files, Acrobat,
    // Gmail's viewer — so it gets the same readable name as the file
    try {
      const M = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
      const dm = /^(\d{4})-(\d{2})-(\d{2})/.exec(String(date || '').slice(0, 10));
      const shortDate = dm ? `${M[Number(dm[2]) - 1]} ${Number(dm[3])} ${dm[1]}` : fmtDate(date);
      const nice = [propName, 'Site Visit Report', walker, shortDate].filter(Boolean).join(' - ');
      doc.setTitle(nice);
      doc.setSubject(`Site visit of ${propName} on ${fmtDate(date)}`);
      if (walker) doc.setAuthor(walker);
      doc.setCreator('SiteVisit IQ'); doc.setProducer('SiteVisit IQ');
    } catch {}
    ...
    // one height for every photo in the report — this is the whole point
    const PH = REC ? 168 : 120;
    const GW = (CW - 20) / 2;
