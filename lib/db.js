...
async function init() {
  ...
  // ---- workbook read-back (DISPLAY ONLY) ----------------------------------
  // The site team's own columns, mirrored off the Site Visit tab of the
  // property's Visit Notes & To-Do workbook, so someone standing at the
  // property can see whether an issue has already been dealt with.
  //
  // OWNERSHIP, which is the whole safety story here:
  //   - the workbook OWNS these four. The app never writes them except from
  //     the read-back job, and the job never writes them back to the workbook.
  //   - the app OWNS title / notes / detail / category / priority /
  //     life_safety. The workbook shows those, frozen at the moment the row
  //     was posted, and nothing in Excel ever changes them here.
  // No field has two owners, so there is never a conflict to resolve. Writes
  // stay one-way; only visibility is two-way.
  //
  // These must never be surfaced as editable inputs in the UI. Editing them in
  // the app would silently diverge from the workbook, which is the exact
  // failure this design exists to avoid.
  await sql`ALTER TABLE items ADD COLUMN IF NOT EXISTS wb_status text`;
  await sql`ALTER TABLE items ADD COLUMN IF NOT EXISTS wb_target_date date`;
  await sql`ALTER TABLE items ADD COLUMN IF NOT EXISTS wb_manager_notes text`;
  await sql`ALTER TABLE items ADD COLUMN IF NOT EXISTS wb_synced_at timestamptz`;
