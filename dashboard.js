...
const RCOLOR = { High: '#e04a54', Low: '#6b7684', Monitor: '#b7791f', Status: '#2a6bd4', Medium: '#6b7684' };
...
function MouseZoom({ src, markers, placing, onPlace, minHeight = 260, fit = 'aspect' }) {
  ...
  return (
    <div ref={wrap} onWheel={onWheel} onMouseDown={down} onMouseMove={mv} onMouseUp={up} onMouseLeave={() => { drag.current = null; }}
      style={{ width: '100%', height: contain ? '100%' : h, borderRadius: contain ? 0 : 10, border: contain ? 'none' : '1px solid #e2e8f0', overflow: 'hidden', position: 'relative', background: '#0a1017', cursor: placing ? 'crosshair' : (tr.s > 1 ? 'grab' : 'zoom-in'), userSelect: 'none' }}>
      <div style={{ transform: `translate(${tr.x}px,${tr.y}px) scale(${tr.s})`, transformOrigin: '0 0', width: '100%', height: contain ? '100%' : 'auto', position: 'relative', display: contain ? 'flex' : 'block', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {(markers || []).map((m, i) => (
          <div key={i} style={{ position: 'absolute', left: m.x + '%', top: m.y + '%', transform: `translate(-50%,-50%) scale(${1 / tr.s})` }}>
            <div style={{ width: 26, height: 26, borderRadius: '50%', background: m.color || '#e04a54', color: '#fff', border: '2px solid #fff', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 6px rgba(0,0,0,.45)' }}>{m.n || ''}</div>
          </div>
        ))}
      </div>
    </div>
...
export default function Dashboard({ user }) {
  ...
  function openProp(id) { setPid(id); setTab('queue'); setView('property'); }
    ...
    <div style={{ minHeight: '100vh', background: '#eef2f6' }}>
      <div style={D.top}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, cursor: 'pointer' }} onClick={() => setView('portfolio')}>
          ...
          <div style={{ color: '#fff', fontWeight: 700, fontSize: 20 }}>SiteVisit <span style={{ color: '#e04a54' }}>IQ</span> <span style={{ color: '#8fa0b4', fontWeight: 500, fontSize: 16 }}>· Dashboard</span></div>
        </div>
        ...
        {view === 'property' && prop && (
          <div>
            ...
            <div style={{ marginTop: 16 }}>
              {tab === 'queue' && <Queue items={propItems} onOpen={setEditItem} patch={patch} onNew={() => setNewOpen(true)} onPhoto={openPhotos} />}
              ...
              {tab === 'archive' && <Archive items={propItems} onOpen={setEditItem} patch={patch} onPhoto={openPhotos} />}
      ...
      {editItem && <ItemModal item={editItem} prop={props.find((p) => p.id === editItem.property_id)} pname={props.find((p) => p.id === editItem.property_id)?.name || editItem.property_id} onClose={() => setEditItem(null)} onSaved={load} onPhoto={openPhotos} onToast={show} />}
  ...
  return (
    <div>
      <div style={D.statsRow}>
        ...
        <Stat n={critN} label="Critical" color="#e04a54" />
        <Stat n={todoN} label="On manager to-do" color="#c8323a" />
      ...
      <div style={D.tabstrip}>
        {TABS.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ ...D.tab, ...(tab === t.id ? D.tabOn : {}) }}>
            {t.label}{badge[t.id] ? <span style={{ ...D.tbadge, ...(t.id === 'critical' ? { background: '#e04a54' } : {}) }}>{badge[t.id]}</span> : null}
          </button>
        ))}
      </div>
    </div>
...
/* ---------------- Queue (card grid) ---------------- */
function Queue({ items, onOpen, patch, onNew, onPhoto }) {
    ...
    <div>
      ...
      {list.length === 0 ? <Empty /> : (
        <div style={D.issueGrid}>
          {list.map((it) => <IssueCard key={it.id} it={it} onOpen={onOpen} patch={patch} onPhoto={onPhoto} />)}
        </div>
      )}
    </div>
  );
...
function IssueCard({ it, onOpen, patch, onPhoto }) {
  const acts = [
      ...
      <div style={D.actionrow}>
        {acts.map(([lbl, on, fn]) => (
          <button key={lbl} onClick={fn} style={D.actbtn}>
            <span style={{ ...D.checkbox, ...(on ? { background: '#e04a54', borderColor: '#e04a54' } : {}) }}>{on ? '✓' : ''}</span>{lbl}
          </button>
        ))}
      ...
      {view.map((it) => (
        <div key={it.id} style={D.row}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: 150, flex: 'none' }}>
            ...
            <button onClick={() => patch(it.id, { send_todo: !it.send_todo })} style={{ ...D.todoBtn, ...(it.send_todo ? { background: '#e04a54', color: '#fff', borderColor: '#e04a54' } : {}) }}>{it.send_todo ? '✓ On to-do' : 'Send to to-do'}</button>
          </div>
        </div>
      ))}
    </div>
...
function ItemModal({ item, prop, pname, onClose, onSaved, onPhoto, onToast }) {
  ...
  return (
    <div style={D.lb} onClick={onClose}>
      <div style={D.modalWide} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', gap: 22, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: '1 1 340px', minWidth: 300 }}>
            ...
            <div onDragOver={(e) => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={(e) => { e.preventDefault(); setDrag(false); addFiles(e.dataTransfer.files); }}
              style={{ ...D.dropzone, ...(drag ? { borderColor: '#e04a54', background: '#fdf1f2' } : {}) }}>
              {photos.length > 0 && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
                  {photos.map((u, i) => <img key={i} src={u} alt="" onClick={() => onPhoto(photos, i, photos.map(() => title))} style={D.modalThumb} />)}
                </div>
              )}
            ...
            {photos.length > 0 && <div style={{ color: '#95a1b0', fontSize: 12.5, marginTop: 5, textAlign: 'center' }}>Click a photo to zoom.</div>}

            <label style={{ ...D.flabel, marginTop: 16 }}>Location on site map</label>
            {prop?.site_map_url ? (
              <div>
                <button style={{ ...D.pinbtn, ...(placing ? { background: '#e04a54', color: '#fff', borderColor: '#e04a54' } : {}) }} onClick={() => setPlacing((p) => !p)}>
          ...
          <div style={{ flex: '1 1 340px', minWidth: 300 }}>
            ...
            <select value={category} onChange={(e) => setCategory(e.target.value)} style={D.finput}>
              <option value="">—</option>
              {CATS.concat(category && !CATS.includes(category) ? [category] : []).map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
            ...
            <label style={D.flabel}>Description</label>
        ...
        {msg && <div style={{ color: '#c8323a', fontSize: 14, marginTop: 10 }}>{msg}</div>}
  ...
  return (
    <div style={D.lb} onClick={onClose}>
      <div style={D.modal} onClick={(e) => e.stopPropagation()}>
        ...
        {msg && <div style={{ color: '#c8323a', fontSize: 14, marginTop: 8 }}>{msg}</div>}
      </div>
    </div>
  );
}
...
const D = {
  ...
  newbtn: { background: '#e04a54', color: '#fff', fontSize: 14, fontWeight: 700, padding: '8px 15px', borderRadius: 8 },
  ...
  critpill: { background: '#fbe3e4', color: '#c8323a', fontSize: 13, fontWeight: 700, padding: '3px 10px', borderRadius: 20 },
  ...
  tabOn: { background: '#fff', color: '#0d1620', border: '1px solid #e2e8f0', borderBottom: '2px solid #e04a54' },
  ...
  ls: { fontSize: 12, fontWeight: 700, color: '#c8323a', background: '#fbe3e4', padding: '1px 7px', borderRadius: 10 },
  lsSm: { fontSize: 10.5, fontWeight: 700, color: '#c8323a', background: '#fbe3e4', padding: '1px 5px', borderRadius: 8, textAlign: 'center' },
  todo: { fontSize: 12, fontWeight: 700, color: '#fff', background: '#e04a54', padding: '1px 7px', borderRadius: 10 },
  ...
  critnum: { width: 24, height: 24, borderRadius: '50%', background: '#e04a54', color: '#fff', fontSize: 13.5, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none' },
  ...
  printbtn: { background: 'rgba(224,74,84,.12)', border: '1px solid rgba(224,74,84,.4)', color: '#c8323a', fontSize: 13, fontWeight: 700, textTransform: 'none', letterSpacing: 0, padding: '7px 13px', borderRadius: 8 },
  // all-issues table
  ...
  modalWide: { background: '#fff', borderRadius: 16, padding: 24, width: 'min(980px,96vw)', margin: 'auto' },
  ...
  modalThumb: { width: 96, height: 96, objectFit: 'cover', borderRadius: 8, border: '1px solid #e6ebf0', cursor: 'zoom-in' },
  ...
  toggleOn: { background: '#e04a54', color: '#fff', borderColor: '#e04a54' },
  toggleLS: { background: '#c8323a', color: '#fff', borderColor: '#c8323a' },
  ...
  adminTag: { fontSize: 11, fontWeight: 700, color: '#c8323a', background: '#fbe3e4', padding: '1px 6px', borderRadius: 8, marginLeft: 8 },
